...
import {effectiveContextWindow, requireContextWindow} from '../../../src/llama/contextWindow.ts';
...
import {
  ...
  replyTokenBudget,
} from '../../../src/contracts/piContext.ts';
import {getModelsIniSectionValues, parseModelsIni} from '../../../src/contracts/modelsIni.ts';
...
test('a snapshot without a known window carries usage but no total', async () => {
  ...
  await database.open();
  try {
    const {ConversationRepository} = await import('../../../src/conversations/repository.ts');
