...
import {
  ...
  blobOidForModelPath,
  parseLocalGguf,
} from '../../../src/models/gguf.ts';
import {ModelCacheRepository} from '../../../src/models/cache.ts';
import {recordModelProps} from '../../../src/llama/modelProps.ts';
...
test('the search asks for the parsed GGUF, and for the sizes it never got', async () => {
  const {HuggingFaceService} = await import('../../../src/models/huggingface.ts');
  const {AppStore} = await import('../../../src/models/store.ts');
  const urls: string[] = [];
