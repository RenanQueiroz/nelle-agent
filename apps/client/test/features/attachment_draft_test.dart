...
void main() {
  test('an added file is uploaded and staged for THIS conversation', () async {
    ...
    expect(
      ...
      contains('conversationId=chat-a'),
    );
  ...
  test(
    'the server refusal is surfaced verbatim, because it names the file',
    () async {
      final c = ProviderContainer(
        overrides: [
          dioProvider.overrideWithValue(
  ...
  test(
    'removing a chip deletes the upload rather than leaving it to be swept',
    () async {
  ...
  test(
    'clear() empties the draft WITHOUT deleting: the uploads are a message now',
    () async {
      final deleted = <String>[];
      final c = ProviderContainer(
        overrides: [
