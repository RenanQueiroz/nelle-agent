...
- `apps/client` (Milestone 5: LAN pairing + auth). A `ServerConnection` is *loopback*
  ...
  - **The refresh lives in `onResponse`, not `onError`.** dio is built with
    ...
    token-rotation bullet below): a 401 for a token another request already replaced
    is *replayed*, not refreshed again.
...
- **CI is the only Mac, Windows and iPhone this project has.** The repo is public, so
  ...
  - **The device suite runs on five platforms**: Linux (under `xvfb`), Windows, macOS,
    ...
    is CI-able as it stands. Linux, Windows and macOS are required; iOS and Android
    remain `continue-on-error` only until each is green with the current harness.
    **Remove that the moment each passes**; a permanently tolerated failure is not a test.
